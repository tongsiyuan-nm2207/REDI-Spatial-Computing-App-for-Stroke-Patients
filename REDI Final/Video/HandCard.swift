...
struct ChoiceCard: View {
    ...
    var body: some View {
        VStack(spacing: 8) {
            Text(title)
                .font(Font.custom("Nohemi-Black", size: 20))
                .fontWeight(.semibold)
                .foregroundStyle(Color(hex: "5237b5"))
            
            Text(subtitle)
                .font(.custom("Nohemi-Regular", size: 15))
                .foregroundStyle(Color(hex: "5237b5").opacity(0.7))
        }
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 32, style: .continuous)
                .stroke(isSelected ? Color(hex: "5237b5").opacity(0.5) : Color(hex: "5237b5").opacity(0.1), lineWidth: isSelected ? 2 : 1)
        )
