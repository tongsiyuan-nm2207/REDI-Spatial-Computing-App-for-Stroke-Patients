...
class TranscriptionManager {
    ...
    // 3. IMPROVED: Enhanced for real-time detection
    func startTranscription(onResult: @escaping (String, Bool) -> Void) async throws {
        transcriber = SpeechTranscriber(
            locale: Locale.current,
            transcriptionOptions: [],
            reportingOptions: [.volatileResults], // IMPORTANT: This enables partial results
            attributeOptions: []
        )
        ...
        recognizerTask = Task {
            for try await result in transcriber!.results {
                ...
                // IMPROVED: Pass both partial and final results immediately
